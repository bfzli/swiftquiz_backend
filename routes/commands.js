...
router.delete("/delete", userAuth, checkRole(["admin"]), async (req, res) => {
        ...
        try {
          await User.find({}).then((users) => {
            ...
            users.forEach(async (user) => {
              await User.findOneAndDelete({ role: "user" });

              await Quiz.findOneAndDelete({ created_by: user._id });
            });
          });
        ...
        try {
          ...
          await User.find({}).then((users) => {
            ...
            users.forEach(async (user) => {
              ...
              await User.findByIdAndUpdate(user._id, { $set: { coins: 0 } });
            });
          });
...
router.put("/add", userAuth, checkRole(["admin"]), async (req, res) => {
